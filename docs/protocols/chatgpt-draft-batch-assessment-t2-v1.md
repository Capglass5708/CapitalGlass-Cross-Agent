# ChatGPT Draft Batch Assessment — T2 Protocol v1
...
- ChatGPT intake: [chat-thread-closeout-autopsy-harvest-chatgpt-v1.md](./chat-thread-closeout-autopsy-harvest-chatgpt-v1.md) (OBSERVED)
- ChatGPT intake: [chat-thread-system-advancement-harvest-chatgpt-v1.md](./chat-thread-system-advancement-harvest-chatgpt-v1.md) (ADVANCEMENT)
...
| --- | --- | --- |
| OBSERVED | `chatgpt-findings-source.md` | Autopsy ChatGPT v1 |
| ADVANCEMENT | `system-advancement-findings-source.md` | System advancement ChatGPT v1 |
...
2. Report `harvestId`, file path, commit SHA.
3. **Stop.** No requirement to run Cursor validate/publish per draft.

Optional (operator):

```bash
npm run harvest:collect-chatgpt-drafts -- --refresh-index
npm run harvest:stage-chatgpt-drafts-on-l
```

Stage copies draft markdown to L: `02-catalog/chatgpt-draft-staging/` and refreshes `00-master-index/BY-KIND/chatgpt-draft-staging-index.json`. **Staging is assessment intake, not hub authority.**
...
npm run harvest:stage-chatgpt-drafts-on-l -- --json
```

Read `work-progress/chatgpt-draft-index.json` or L: `02-catalog/chatgpt-draft-staging/chatgpt-draft-index.json`. Select drafts for this batch (by date, theme, or lane).
...
| `npm run harvest:stage-chatgpt-drafts-on-l` | Copy draft markdown to L: staging catalog + index slice |
