 ...
 * Does not validate or publish — refreshes work-progress/chatgpt-draft-index.json.
 *
 * Usage:
...
function main() {
  ...
  if (json) {
    ...
    console.log("  next: npm run harvest:stage-chatgpt-drafts-on-l (default before batch assessor)");
  }
}
