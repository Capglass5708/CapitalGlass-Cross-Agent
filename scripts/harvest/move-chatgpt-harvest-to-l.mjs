...
function main() {
  ...
  try {
    ...
    if (args.json) {
      ...
      console.log(`  moved=${receipt.movedCount} skipped=${receipt.skippedCount} failures=${receipt.failureCount}`);
      if (repoReceiptPath) {
        console.log(`  repoReceipt=${path.relative(REPO_ROOT, repoReceiptPath)}`);
      }
      if (receipt.verdict === PASS_VERDICT) {
        console.log("  verification: source absent after move; destination hashes match");
      }
