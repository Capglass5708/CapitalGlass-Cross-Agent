...
/**
 * Compute legacy-path operational verdict — never unqualified OPERATIONAL when a required layer was skipped.
 */
export function computeLegacyPublicationVerdict({
  ...
  if (layers.supabase && supabaseProjection?.ok === false && supabaseProjection?.code !== "SKIP") {
    // Capability-unavailable skips are degraded, not hard-fail, for T1/T2.
    if (supabaseProjection?.code === "SUPABASE_CAPABILITY_UNAVAILABLE") {
      degradedReasons.push("SKIPPED_REQUIRED:supabase_capability_unavailable");
    } else {
