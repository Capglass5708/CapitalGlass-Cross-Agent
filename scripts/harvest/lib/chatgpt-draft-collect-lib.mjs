...
export const DRAFT_FILES = [
  {
    file: "chatgpt-findings-source.md",
    lane: "OBSERVED",
...
function inferVerdictFromMarkdown(content) {
  const patterns = [
    /Output verdict:\s*`?([A-Z0-9_]+)`?/i,
    /verdict:\s*`?(CHATGPT_SOURCE_PUBLISHED|BLOCKED_GIT_PUBLICATION|DRAFT_READY|SYSTEM_ADVANCEMENT_DRAFT_READY)`?/i,
  ];
...
export function buildChatgptDraftIndex(drafts) {
  return {
    ...
    gitPublicationContract: "CHATGPT-HARVEST-GIT-PUBLICATION-CONTRACT-V1",
    defaultTier: "T2",
    authorityNote:
      "Draft markdown on chat-gpt-harvest is Git staging — not scout truth or operational hub authority until Cursor validates.",
    counts: {
      total: drafts.length,
    ...
    nextBatchAssessor: {
      ...
      openerProtocol: "harvest/protocol/CHATGPT-DRAFT-BATCH-ASSESSMENT-T2-V1.md",
