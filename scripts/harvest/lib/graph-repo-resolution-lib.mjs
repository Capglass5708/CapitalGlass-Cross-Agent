...
/**
 * Resolve CG-MASTER-GRAPH root without hardcoding host-local paths in receipts.
 */
export function resolveGraphRepoRoot(crossAgentRoot, options = {}) {
  ...
  if (
    process.env.CG_MASTER_GRAPH_ROOT &&
    fs.existsSync(path.join(process.env.CG_MASTER_GRAPH_ROOT, "package.json"))
  ) {
    return {
      ok: true,
