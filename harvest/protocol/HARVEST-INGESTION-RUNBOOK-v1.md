...
| 2 | `npm run harvest:sync-derived` | CapitalGlass-Cross-Agent | Regenerate compact records, receipt, packet-index, coverage; **prompt extraction**; refresh packet registry SHAs |
| 2b | `npm run harvest:sync-z-mirror` | CapitalGlass-Cross-Agent | Mirror protocol docs to `harvest/` + `Z:\Capital-Glass-Dev\Harvest` when Z: mounted |
...
### Prompt extraction (automatic in step 2)

`harvest:sync-derived` runs prompt candidate extraction and writes per-harvest artifacts under `artifacts/agent-runs/<harvest-id>/`. Approved prompts require operator IDs in `manifest.promptHarvest.operatorApprovals`, then re-sync.

| Step | Command | Repo | Purpose |
| --- | --- | --- | --- |
| P1 | Re-run `npm run harvest:sync-derived -- <harvest-id>` | Cross-Agent | Promote approved candidates; update `work-progress/harvest-prompt-catalog-delta.json` |
| P2 | `npm run promptops:project-harvest-prompts` | CG-AppBuilder-MCP | Idempotent Supabase seed for approved prompts |
| P3 | `npm run prompt-catalog:compile-index` | CG-AppBuilder-MCP | Merge harvest deltas into hot-cache prompt catalog |

Full operational publish (`harvest:publish-intelligence-full`) also runs `harvest:sync-z-mirror`, upserts `BY-KIND/prompt-harvest-index.json`, and calls `promptops:project-harvest-prompts` when Supabase projection is enabled.

Z: operator protocol root: `Z:\Capital-Glass-Dev\Harvest\protocol\` (git authority: `CapitalGlass-Cross-Agent/harvest/`).
