...
$cmds = @(
    "echo '$ea' | sudo -S grep -v '^#' /etc/ssh/sshd_config | grep -v '^$'",
    "echo '$ea' | sudo -S ls /etc/ssh/sshd_config.d 2>&1",
    "echo '$ea' | sudo -S grep -r 'Syno\\|syno\\|AllowUsers\\|Match' /etc/ssh/ 2>/dev/null"
)
foreach ($c in $cmds) {
