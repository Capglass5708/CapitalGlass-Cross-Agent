...
$cmds = @(
    "echo '$ea' | sudo -S /usr/syno/sbin/synoapppriv_updater --help 2>&1 | head -30",
    "echo '$ea' | sudo -S grep -r 'terminal\\|ssh' /usr/syno/etc/privilege* 2>/dev/null | head -20",
    "echo '$ea' | sudo -S find /usr/syno/etc -name '*priv*' 2>/dev/null | head -15"
)
foreach ($c in $cmds) {
