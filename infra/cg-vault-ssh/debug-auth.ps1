...
$cmds = @(
    "echo '$ea' | sudo -S cat /volume1/docker/cg-vault-ssh/authorized_keys",
    "echo '$ea' | sudo -S ls -la /volume1/docker/cg-vault-ssh/authorized_keys /volume1/docker/cg-vault-ssh/bin/cg-vault-run.sh",
    "echo '$ea' | sudo -S getent passwd cg-context-ledger",
    "echo '$ea' | sudo -S tail -20 /var/log/auth.log 2>/dev/null || echo '$ea' | sudo -S tail -20 /var/log/messages 2>/dev/null | grep sshd"
)
foreach ($c in $cmds) {
