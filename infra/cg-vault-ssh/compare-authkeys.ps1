...
$cmds = @(
    "echo '$ea' | sudo -S cat /var/services/homes/cg-context-ledger/.ssh/authorized_keys",
    "echo '$ea' | sudo -S wc -c /volume1/docker/cg-vault-ssh/authorized_keys /var/services/homes/cg-context-ledger/.ssh/authorized_keys"
)
foreach ($c in $cmds) {
