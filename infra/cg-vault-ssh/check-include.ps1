...
$r = Invoke-SSHCommand -SessionId $session.SessionId -Command "echo '$ea' | sudo -S grep -n Include /etc/ssh/sshd_config; echo '$ea' | sudo -S ls -la /etc/ssh/sshd_config.d 2>&1" -TimeOut 60
