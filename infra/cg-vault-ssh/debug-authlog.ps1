...
$cmds = @(
    "echo '$ea' | sudo -S /usr/bin/sshd -T -f /volume1/docker/cg-vault-ssh/sshd_config 2>&1 | grep -iE '^usepam|^authorizedkeys|^match|^include'",
    "echo '$ea' | sudo -S grep -iE '22222|Failed publickey|cg-context|AuthorizedKeys' /var/log/auth.log 2>/dev/null | tail -25",
    "echo '$ea' | sudo -S cat /etc/ssh/sshd_config 2>&1 | head -40"
)
foreach ($c in $cmds) {
