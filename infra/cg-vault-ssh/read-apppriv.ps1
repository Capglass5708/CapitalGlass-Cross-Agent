...
$cmds = @(
    "echo '$ea' | sudo -S cat /var/services/homes/cg-context-ledger/.ssh/APPPRIV_FIX.txt",
    "echo '$ea' | sudo -S cat /var/services/homes/cg-context-ledger/.ssh/SYNOUSER_CONF.txt",
    "echo '$ea' | sudo -S cat /var/services/homes/cg-context-ledger/.ssh/APPRIV_GET.txt"
)
foreach ($c in $cmds) {
