...
$b64 = [Convert]::ToBase64String([IO.File]::ReadAllBytes((Join-Path $PSScriptRoot 'test-echo-authkeys-remote.sh')))
$cmd = "echo '$ea' | sudo -S sh -c 'echo $b64 | base64 -d > /tmp/test-echo-ak.sh && sh /tmp/test-echo-ak.sh'"
