...
$cmds = @(
    "echo '$ea' | sudo -S ls -la /volume1/docker/cg-vault-ssh/bin/cg-vault-run.sh",
    "echo '$ea' | sudo -S -u cg-context-ledger env SSH_ORIGINAL_COMMAND=VAULT_SSH_PROBE /volume1/docker/cg-vault-ssh/bin/cg-vault-run.sh",
    "echo '$ea' | sudo -S tail -5 /var/log/auth.log 2>/dev/null"
)
foreach ($c in $cmds) {
