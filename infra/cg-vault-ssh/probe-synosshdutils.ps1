...
$cmds = @(
    "echo '$ea' | sudo -S /usr/syno/sbin/synosshdutils --help 2>&1",
    "echo '$ea' | sudo -S /usr/syno/sbin/synosshdutils 2>&1 | head -20",
    "echo '$ea' | sudo -S strings /usr/bin/sshd 2>/dev/null | grep -i syno | head -20"
)
foreach ($c in $cmds) {
