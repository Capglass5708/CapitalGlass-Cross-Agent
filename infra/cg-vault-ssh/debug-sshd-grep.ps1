...
$r = Invoke-SSHCommand -SessionId $session.SessionId -Command "echo '$ea' | sudo -S /usr/bin/sshd -T -f /volume1/docker/cg-vault-ssh/sshd_config 2>&1 | grep -iE 'authorizedkeys|allowusers|authenticationmethods|pubkey|match'" -TimeOut 60
