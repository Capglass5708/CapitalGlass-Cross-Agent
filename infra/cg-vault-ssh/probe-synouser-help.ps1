...
$cmds = @(
    "echo '$ea' | sudo -S /usr/syno/sbin/synouser --help 2>&1 | head -40",
    "echo '$ea' | sudo -S cat /var/services/homes/cg-context-ledger/.ssh/SYNOUSER_HELP.txt 2>&1 | head -40",
    "echo '$ea' | sudo -S cat /var/services/homes/cg-context-ledger/.ssh/SSH_ENABLE.txt 2>&1"
)
foreach ($c in $cmds) {
