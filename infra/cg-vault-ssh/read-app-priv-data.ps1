...
$cmds = @(
    "echo '$ea' | sudo -S /usr/syno/bin/user.data.collector/synouserdata_app_priv 2>&1 | head -60",
    "echo '$ea' | sudo -S grep -r 'enable_ssh\\|ssh_user\\|allow.*ssh\\|terminal' /etc/ssh /usr/syno/etc/ssh* 2>/dev/null | head -20"
)
foreach ($c in $cmds) {
