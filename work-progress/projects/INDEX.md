...
| `capital-glass-operations-command-center-v1` | `intel:45d4fa89e72145…` | WARN | PASS | CapitalGlass-CalendarDashBoard | `harvest-2026-08-17-capital-glass-operations-command-center-v1` | see capital-glass-operations-command-center-v1.md |
...
_238 registry packets · 106 intelligence entities_
...
| --- | --- | --- | --- | --- | --- |
| `context-ledger-phase-0-authority-resolution-v1` | [2026-08-30_context-ledger-phase-0-authority-resolution-v1.md](./2026-08-30_context-ledger-phase-0-authority-resolution-v1.md) | **BLOCKED** — `CG_CONTEXT_LEDGER_PHASE_0_AUTHORITY_V1_BLOCKED`. 5 of 7 items resolved; Claude corpus preserved (502 files, PASS). No approved storage location is durable + authorized for secret-bearing raw payloads + policy-governed. | CapitalGlass-Cross-Agent | `db7edda` | Operator decision on canonical Evidence object-storage target |
| `shared-checkout-git-state-unprotected-v1` | [2026-08-30_shared-checkout-git-state-unprotected-v1.md](./2026-08-30_shared-checkout-git-state-unprotected-v1.md) | **OPEN — governance defect, BLOCKING for concurrent-agent mutation.** The checkout lease arbitrates file edits only; the Git index and HEAD/refs of a shared checkout are unprotected. Five live sessions shared one checkout; a `git checkout -b` redirected three unrelated commits onto another mission's branch. No corruption, but branch ownership was violated. | CG-AppBuilder-MCP (lease impl); CapitalGlass-Cross-Agent (finding) | This commit | Adversarial proof that a second agent is DENIED branch/index mutation in another's checkout and ALLOWED its own worktree |
...
| `wesleywork-storage-protocol-contradiction-remediation-v1` | [2026-08-13_wesleywork-storage-protocol-contradiction-remediation-v1.md](./2026-08-13_wesleywork-storage-protocol-contradiction-remediation-v1.md) | **LIVE_MACHINE_PROOF_PASS** — ProgramData tombstone verified | CapitalGlass-Office-Admin, CapitalGlass-Cross-Agent | — | Hub republish via WESLEYDESK GHA. Do not write L: from Cursor. |
...
| Project ID | Status | Next action |
| --- | --- | --- |
| `wesleywork-storage-protocol-contradiction-remediation-v1` | LIVE_MACHINE_PROOF_PASS | Hub republish via WESLEYDESK GHA; Storage Keeper only |
...
| Project ID | Status | Next action |
| --- | --- | --- |
| `cg-federated-repo-index-v1` | **WAVE_A_CLOSED / LIVE_RYZEN9_PROOF_PASS** | Bound to AppBuilder `c87ee2420`; Hub L: publish last |
