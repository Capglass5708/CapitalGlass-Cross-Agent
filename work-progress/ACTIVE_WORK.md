...
| Current focus | Proposal Generator door + hardware money per estimate section (`pg-estimate-sections-door-money-rollups-v1`) — implemented and pushed, Cursor-ProposalGenerator PR #228 open (draft), watched; prior: WESLEYWORK Storage Keeper sole repair front door (`wesleywork-storage-protocol-contradiction-remediation-v1`)
...
### 2026-09-04 CT — pg-estimate-sections-door-money-rollups-v1 (implemented, PR #228 open)

| Field | Value |
| --- | --- |
| Work package | `pg-estimate-sections-door-money-rollups-v1` |
| Status | **IMPLEMENTED — Cursor-ProposalGenerator PR #228 open (draft), watched.** Superseded the earlier 5-PR plan after Wesley rejected its scope. |
| Repos involved | CapitalGlass-Cross-Agent (project file + ledger); Cursor-ProposalGenerator (owner and sole execution repo) |
| Notes | First pass over-scoped Wesley's ask ("PDF roll-ups need Interior/Exterior/Retail; door module hardware pricing follows each door into its section") into a 5-PR governance program with five open decisions. Wesley rejected it directly ("I didn't ask for all this what are you doing?"), named Smith Ranch as the pilot project, and gave a direct go-ahead to push to Cursor-ProposalGenerator. Before writing code, confirmed Smith Ranch's real state by querying the live `Capital Glass DataBase` Supabase project directly: project `4402eb63-3b81-465e-8220-f30cd0854509` (`CG-2037-26`, "Smith Ranch Road") is already `dynamic_sections_v1` with 3 active sections (Exterior/Interior/Retail) and 17 real doors, but `pricingBySectionId` and `estimateSectionRecordAssignments` were both completely empty — confirming the original diagnosis (no write path bridges door money into the section ledger) as the actual, narrow root cause, not a speculative one. Rather than build the persisted-ledger write path from the original plan (which would need new OCC/reassignment/erasure-guard plumbing inside a canonical multi-section autosave transaction this session doesn't own, on a live financial document), implemented a lower-risk **read-time overlay**: `computeDoorSectionMoney`/`mergeDoorMoneyIntoSectionLedger` fold each door's material+fabrication+hardware+labor onto its resolved section fresh on every read, using the legacy door-scope markup/tax formula so pricing matches across models. Threaded through the one financial path every consumer shares (calculate → markup report → document model → output bundle → intelligence document, plus the interactive Markup Report page) so the PDF preview, Markup Report, and project total all agree. Also fixed issue-readiness (`buildDoorRecordAttributions`) to use the full money formula instead of `fabricationCost` alone. Verified locally in Cursor-ProposalGenerator: `npm run test:all`, `typecheck` (0 errors), `lint` (0 errors), `build` all green; 9 new tests plus all 25 pre-existing elevations-doors tests pass (34/34); one pre-existing unrelated test failure confirmed to reproduce identically on unmodified `origin/main`. |
| Evidence | Project file `work-progress/projects/2026-09-04_pg-estimate-sections-door-money-rollups-v1.md` (updated with full detail); Cursor-ProposalGenerator PR #228 (`claude/smith-ranch-section-door-money-9j0pgk`, commit `1c14ea8`) |
| Next | Wesley reviews/merges PR #228; then prices at least one door per section on Smith Ranch to see real Interior/Exterior/Retail dollars on the PDF preview. Session is watching PR #228 for CI/review events. |
