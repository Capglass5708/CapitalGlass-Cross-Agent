...
**Status:** IMPLEMENTED on `work/context-ledger-clean-forward-v1`. This spec was
authored as DESIGN ONLY against `work/estate-text-capture-v1`, whose lane authority
was never granted and whose lineage is now frozen as
`HISTORICAL_SUPERSEDED_UNPUSHABLE`. The binding is implemented instead in
`scripts/context-ledger/lib/quantitative-prose.mjs`, enforced by
`scripts/tests/run-quantitative-prose-binding.test.mjs`.

Every `~4.7 MB/s` reference below is **retraction/history metadata** describing the
original defect. It is not a claim, and the guard refuses it as an active value.

**Defect class:** `QUANTITATIVE_PROSE_EMITTER` — **CONFIRMED, REMEDIATED**
